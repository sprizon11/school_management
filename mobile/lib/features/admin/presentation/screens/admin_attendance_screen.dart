...
class _AdminAttendanceScreenState extends ConsumerState<AdminAttendanceScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      final res =
          await ref.read(dioProvider).get('/admin/attendance/overview');
      if (!mounted) return;
      ...
      setState(() {
        _data = data;
        _selectedClassId =
            classes.isNotEmpty ? '${(classes.first as Map)['id']}' : null;
        _loading = false;
      });
  ...
  int get _overallPercent {
    ...
    final sum = _weekly.fold<int>(0, (a, p) => a + _toInt((p as Map)['percent']));
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          _gradientButton('Mark Attendance', Icons.event_available_rounded,
  ...
  Widget _statsRow() {
    ...
    return SizedBox(
      ...
      child: ListView(
        ...
        children: [
          _statCard(Icons.groups_rounded, _green, '$_overallPercent%',
              'Overall Attendance', 'This Month'),
          const SizedBox(width: 12),
          _statCard(Icons.how_to_reg_rounded, _blue,
              '${_toInt(_today['present'])}', 'Present Today', 'of $total'),
          const SizedBox(width: 12),
          _statCard(Icons.person_off_rounded, _red,
              '${_toInt(_today['absent'])}', 'Absent Today', 'of $total'),
          const SizedBox(width: 12),
          _statCard(Icons.event_available_rounded, _amber,
              '${_classes.length}', 'Classes Taken', 'Today'),
        ],
      ),
    );
  }
  ...
  Widget _statCard(
      IconData icon, Color color, String value, String label, String sub) {
    return Container(
      width: 150,
  ...
  Widget _dateFilterRow() {
    return Row(
      children: [
        Expanded(
          child: _pill(Icons.calendar_today_rounded, 'Date',
              '${DateFormat('d MMM yyyy').format(DateTime.now())} (Today)'),
        ),
        const SizedBox(width: 10),
        Expanded(
          child: _pill(Icons.tune_rounded, 'Filter', 'All Classes'),
        ),
      ],
    );
  }
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text('${c['name'] ?? 'Class ${c['grade']}${c['section']}'}',
                      maxLines: 1,
                      overflow: TextOverflow.ellipsis,
                      style: TextStyle(
                          fontSize: 12,
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              Expanded(
                child: Text('$name Attendance Overview',
                    style: const TextStyle(
                        fontSize: 14.5,
                        fontWeight: FontWeight.w800,
                        color: _ink)),
        ...
        children: [
          ...
          Column(
            ...
            children: [
              Text(label,
                  style: TextStyle(
                      fontSize: 11, color: _ink.withValues(alpha: 0.55))),
              Text(value,
                  style: TextStyle(
                      fontSize: 12,
                      fontWeight: FontWeight.w800,
  ...
  Widget _studentsHeader() {
    return Row(
      children: [
        ...
        const Text('Roll No.',
            style: TextStyle(
                fontSize: 12.5, fontWeight: FontWeight.w700, color: _purple)),
  ...
  Widget _studentsList() {
    ...
    if (_roster.isEmpty) {
      return Container(
        ...
        decoration: _cardDecoration(),
        child: const Text('No students in this class yet.',
            style: TextStyle(color: _grey)),
      );
    }
    final sorted = [..._roster]..sort(
        (a, b) => _toInt((a as Map)['rollNumber'])
